...
var endpoint = `https://gateway.marvel.com:443/v1/public/characters?orderBy=name&limit=100&offset=1&ts=${ts}&apikey=${pubkey}&hash=${hash}`
...
function carregar_hero() {
    ...
    ajax.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            data_json = JSON.parse(this.responseText).data.results;
...
function printCard() {
    ...
    if (data_json.length > 0) {
        ...
        html_content = msg_alert("No hero found", "dark");
        content.innerHTML = html_content;
    }
}
